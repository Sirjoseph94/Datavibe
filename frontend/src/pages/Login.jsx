  ...
  return (
    <div className={`min-h-screen flex items-center justify-center p-4 relative overflow-hidden transition-colors duration-300 ${
      ...
      <div className={`w-full max-w-md p-8 rounded-3xl border shadow-xl transition-all duration-300 relative z-10 ${
        ...
        <h2 className="text-2xl font-black mb-1 text-center">Welcome Back</h2>
        <p className={`text-xs text-center mb-8 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>Sign in to manage your data subscriptions</p>
        ...
        <div className={`mt-6 text-center text-xs ${theme === 'dark' ? 'text-slate-405' : 'text-slate-505'}`}>
          Don't have an account? <Link to="/register" className="text-emerald-500 hover:text-emerald-650 font-bold ml-1">Register</Link>
        </div>
      </div>
    </div>
  );
